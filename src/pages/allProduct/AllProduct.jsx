...
const AllProduct = () => {
    ...
    const filterData = getAllProduct.filter((obj) => obj.selectCategory.toLowerCase().includes(filterByCategory))
        .filter((obj) => obj.selecteHeelHeight.toLowerCase().includes(filterByHeelHeight))
        ...
        .filter((obj) => obj.selectMaterial.toLowerCase().includes(filterByMaterial))
        .filter((obj) => obj.selectToeStyle.toLowerCase().includes(filterByToeStyle))
                                                    ...
                                                    <div className="h-full border border-gray-300 rounded-xl overflow-hidden shadow-md cursor-pointer">
                                                        ...
                                                        <div className="p-6">
                                                            ...
                                                            {cartItems.some((p) => p.id === item.id) ?
                                                                <Button onClick={() => deleteCart(item)} className=" bg-red-500 hover:bg-red-600 w-full text-white py-2.5 rounded-lg font-bold">
                                                                    Delete From Cart
                                                                </Button>
                                                                :
                                                                <Button onClick={() => addCart(item)} className=" bg-indigo-500 hover:bg-indigo-600 w-full text-white py-2.5 rounded-lg font-bold">
                                                                    Add To Cart
                                                                </Button>}
                                                        </div>
                                                        </div>
